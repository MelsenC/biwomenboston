        ...
        transition: {
            open: function(v, dfr) {
                var l = v.get_layout().hide()
                  ...
                  , thm = this
                  , d = v.dom_get();
                return d.find(".slb_content").css({
                    width: "",
                    height: ""
                }).find(this.get_tag_selector()).hide(),
                d.find(".slb_details").height(0),
                d.show({
                    always: function() {
                        var pos = {
                            top_base: $(document).scrollTop()
                        };
                        document.documentElement.clientWidth > thm.get_breakpoint("small") ? (pos.top = pos.top_base + $(window).height() / 2 - l.height() / 2,
                        pos.top < pos.top_base && (pos.top = pos.top_base)) : pos.top = pos.top_base,
                        o.fadeIn({
                            always: function() {
                                l.css(pos),
                                dfr.resolve()
            ...
            load: function(v) {
                return v.get_layout().find(".slb_loading").show(),
                document.documentElement.clientWidth > this.get_breakpoint("small") ? v.get_layout().fadeIn().promise() : v.get_layout().show().promise()
            },
