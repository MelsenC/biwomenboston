window.SLB && SLB.has_child("View.extend_template_tag_handler") && !function() {
    SLB.View.extend_template_tag_handler("ui", {
        ...
        events_init: function(ev) {
            ...
            v.on("slideshow-toggle", function(ev, v) {
                ...
                if (tags.length)
                    for (var render_tag = function(tag) {
        ...
        render: function(item, tag, dfr) {
            ...
            return this.util.is_promise(ret) ? ret.done(function(output) {
                dfr.resolve(output)
            }) : dfr.resolve(ret),
            dfr.promise()
        },
        props: {
            ...
            group_status: function(item) {
                if (item.get_group().is_single())
                    return "";
                var key, ph, out = item.get_viewer().get_label("group_status"), delim = "%", handlers = {
                    current: function() {
                ...
                for (key in handlers)
                    ph = delim + key + delim,
                    -1 !== out.indexOf(ph) && (out = out.replace(new RegExp(ph,"ig"), handlers[key]()));
                return out
            }
        }
    })
